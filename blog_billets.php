...
<?php
	...
	//recuperation du GET 'tous'
	//pour réafficher tous les billets
	if (isset($_GET['tous'])){
		$tous = htmlspecialchars($_GET['tous']);
		if ($tous='tous'){
			$_SESSION['id_demande'] = NULL;
		}
		if ($_SESSION['debug'] == 1) {echo"<p class='debug'>\$tous = ".$tous."</p>";}
		if ($_SESSION['debug'] == 1) {echo"<p class='debug'>\$_SESSION['id_demande'] = ".$_SESSION['id_demande']."</p>";}
	}
...
<head>
		<meta charset="utf-8" />
		<title>Blog - affichage des billets</title>
...
<form action="blog_billets_post.php" method="post">
...
<div id="tous_billets">
<a href="blog_billets.php?tous=tous">Voir tous les billets</a><br />
</div>
...
if (!($donneestest['nombre_total'] == 0)){		//si le nbre d'enregistrements est diff de zero
...
if (isset($id_billet_demande)){
	...
	if ($_SESSION['debug'] == 1) {echo "<p class='debug'>120 - \$strQuery = ".$sqlQuery."</p>";}
	
	$reponse = $bdd -> query($sqlQuery);
	while ($donnees = $reponse->fetch()){
		if ($_SESSION['debug'] == 1) {echo "<p class='debug'>124 present_id = ".$donnees['nb_billets']."</p>";}
		...
		if (($donnees['nb_billets']) == 1){		
				...
				$sqlQuery = 'SELECT *, DATE_FORMAT(date_crea, \'%d/%m/%Y à %Hh %imn %ss\') AS date_creation_fr 
							FROM billets 
							WHERE id =' . $id_billet_demande . '';
				if ($_SESSION['debug'] == 1) {echo "<p class='debug'>132 - Le \$id_billet_demande existe bien !</p>";}	
		}
		else{
...
<a href="blog_commentaires.php?id=<?php echo $donnees['id'];?>"> Lire les commentaires ... et commenter ! </a>
</div>
...
else{	//if (!($donneestest['nombre_total'] == 0)){
